...
The Phone App's Job: 1. Acts as the buffer, holding chunks until it forms 2 to 10-second windows.
...
Authenticates the user securely via standard Firebase Auth.

Uses JavaScript ArrayBuffers to capture the 100ms binary stream and convert it to float16.

Stitches chunks into 2 to 10-second windows and pushes them to Firebase via the JS SDK.
